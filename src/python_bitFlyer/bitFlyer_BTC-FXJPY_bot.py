...
#
    ...
    def calculate_lot(self, df_candleStick, margin, risk, term ):
        ...
        lot = round(able_lot,2)        #print("許容リスクから購入できる枚数は最大{}BTCまでです".format(calc_lot))
...
#
                ...
                #ショートエントリー 
                if judgement[1] and short_order_count < self.max_orders and onLoop == True and isRange[-1]!=True and (lastPositionBid == 0.00 or lastPositionBid <= best_bid):
                    counter = 0
                    short_order_count += 1
                    ...
                    if (profitPos > self.cost*self.lot*self.margin):
                        logger.info("Short Entry Profit=" + "{:.2f}".format(profitPos))
                    logger.info("Short Entry Position=" + "{:.2f}".format(profitPos))
                    okOrder = False
                    ...
                    okOrder = True
                    pos -= 1
                    message = "Short entry Lot=" + "{:.2f}".format(lot) + "@price={}".format(best_bid)
                    ...
                    time.sleep(10)
                #ロングエントリー
                if judgement[0] and long_order_count < self.max_orders and onLoop == True and isRange[-1]!=True and (lastPositionAsk == 9999999.99 or lastPositionAsk >= best_ask):
                    counter = 0
                    long_order_count += 1
                    lot = self.calculate_lot(df_candleStick, self.margin, self.risk, entryTerm) 
                    if (lot < self.lot):
                    ...
                    if (profitPos > self.cost*self.lot*self.margin):
                        logger.info("Long Entry Profit=" + "{:.2f}".format(profitPos))
                    logger.info("Long Entry Position=" + "{:.2f}".format(profitPos))
                    okOrder = False
                    print(datetime.datetime.now())
                    ...
                    pos += 1
                    okOrder = True
                    message = "Long entry Lot=" + "{:.2f}".format(lot) + "@price={}".format(best_ask)
                    self.lineNotify(message)
                    logger.info(message)
                    if (lastPositionAsk == 9999999.99 or lastPositionAsk > best_ask):
                        lastPositionAsk = best_ask
                    ...
                    time.sleep(10)
#
                if judgement[2] and long_order_count > 0 and onLoop == True and self.my_round((best_bid - lastPositionBid),2) > getProfit:
                    #ロングクローズ
                    counter = 0
                    plRange = best_bid - lastPositionBid
                    pl.append(pl[-1] + plRange * lot)
                    profitPos = pl[-1]
                    logger.info("Long Close Profit=" + "{:.2f}".format(profitPos))
                    okOrder = False
                    print(datetime.datetime.now())
                    ...
                    message = "bitFlyer_Bot(ETHBTC) Long Close Lot:{:.2f}, Price:{}, pl:{:.2f}, Result:{}".format(long_order_lots, best_ask, profitPos, mes)
                    ...
                    onLoop = False
                    time.sleep(10)
                if judgement[3] and short_order_count > 0 and onLoop == True and self.my_round((lastPositionAsk - best_ask),2) > getProfit:
                    #ショートクローズ
                    counter = 0
                    plRange = lastPositionAsk - best_ask
                    pl.append(pl[-1] + plRange * lot)
                    ...
                    message = "bitFlyer_Bot(ETHBTC) Short Close Lot:{:.2f}, Price:{}, pl:{:.2f}, Result:{}".format(short_order_lots, best_bid, profitPos, mes)
                    ### comment because od file sizr bigger fileName = self.describePLForNotification(pl, df_candleStick)
                    ### comment because od file sizr bigger self.lineNotify(message,fileName)
                    logger.info(message)
                    self.lineNotify(message)
...
######
                ### ロングクローズ()
                if (long_order_count > 0 and counter > 30 and self.my_round((best_bid - lastPositionBid),2) > getProfit and onLoop == True):
                    # ロングクローズ
                    counter = 0
                    plRange = best_bid - lastPositionBid
                    pl.append(pl[-1] + plRange * lot)
                    ...
                    time.sleep(10)
                #### ショートクローズ
                if (short_order_count > 0 and counter > 30 and self.my_round((lastPositionAsk - best_ask),2) > getProfit and onLoop == True):
                    # ショートクローズ
                    counter = 0
                    plRange = lastPositionAsk - best_ask
                    pl.append(pl[-1] + plRange * lot)
